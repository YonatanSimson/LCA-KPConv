...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Hugues THOMAS - 11/06/2018, modified by Onur Bayrak
#
...
class STPLS3DDataset(PointCloudDataset):
        ...
        for root, dirs, files in os.walk(self.path):
            for file in files:
                if file.endswith(".ply"):
                    ...
                    cloud = np.vstack((cloud['x'], cloud['y'], cloud['z'],cloud['red'], cloud['green'], cloud['blue'],cloud['scalar_class'])).T
                    ...
                    labels = cloud[:, 6].astype(np.float32)

                    write_ply(cloud_file, (xyz, colors, labels), ['x', 'y', 'z', 'red', 'green', 'blue', 'scalar_class'])
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                write_ply(sub_ply_file,
                          ...
                          ['x', 'y', 'z', 'red', 'green', 'blue', 'scalar_class'])
    ...
    def load_evaluation_points_label(self, file_path):
        ...
        # Get original points
        data = read_ply(file_path)
        return np.vstack((data['scalar_class'])).T
