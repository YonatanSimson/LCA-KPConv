...
static PyObject* batch_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	if ((int)lca_PyArray_NDIM(points_array) != 2 || (int)lca_PyArray_DIM(points_array, 1) != 3)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(classes_array);
	...
	if ((int)lca_PyArray_NDIM(batches_array) > 1)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(classes_array);
	...
	if (use_feature && ((int)lca_PyArray_NDIM(features_array) != 2))
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(classes_array);
	...
	if (use_classes && (int)lca_PyArray_NDIM(classes_array) > 2)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(classes_array);
	...
	// Number of points
	int N = (int)lca_PyArray_DIM(points_array, 0);

	// Number of batches
	int Nb = (int)lca_PyArray_DIM(batches_array, 0);

	// Dimension of the features
	int fdim = 0;
	if (use_feature)
		fdim = (int)lca_PyArray_DIM(features_array, 1);

	//Dimension of labels
	int ldim = 1;
	if (use_classes && (int)lca_PyArray_NDIM(classes_array) == 2)
		ldim = (int)lca_PyArray_DIM(classes_array, 1);

	// Check that the input array respect the number of points
	if (use_feature && (int)lca_PyArray_DIM(features_array, 0) != N)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(classes_array);
	...
	if (use_classes && (int)lca_PyArray_DIM(classes_array, 0) != N)
	{
		Py_XDECREF(points_array);
		Py_XDECREF(batches_array);
		Py_XDECREF(classes_array);
	...
	original_points = vector<PointXYZ>((PointXYZ*)lca_PyArray_DATA(points_array), (PointXYZ*)lca_PyArray_DATA(points_array) + N);
	original_batches = vector<int>((int*)lca_PyArray_DATA(batches_array), (int*)lca_PyArray_DATA(batches_array) + Nb);
	if (use_feature)
		original_features = vector<float>((float*)lca_PyArray_DATA(features_array), (float*)lca_PyArray_DATA(features_array) + N * fdim);
	if (use_classes)
		original_classes = vector<int>((int*)lca_PyArray_DATA(classes_array), (int*)lca_PyArray_DATA(classes_array) + N * ldim);
	...
	memcpy(lca_PyArray_DATA(res_points_obj), subsampled_points.data(), size_in_bytes);
	size_in_bytes = Nb * sizeof(int);
	memcpy(lca_PyArray_DATA(res_batches_obj), subsampled_batches.data(), size_in_bytes);
	if (use_feature)
	{
